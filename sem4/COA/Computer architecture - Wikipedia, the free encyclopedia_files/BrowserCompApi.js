/*
* NEW BrowserCompAPI version 1.0.1.4
...
context = JSON.parse(abstractionlayer.commons.appMethods.getContext(id).result);
}
...
return BcApiObj.CrossDomainHttpRequest(callback, method, url, postParams, userName, password, headersArr);
};
...
wasCrossDomainHTTPRequestDataReturned(objInnerText.response, objInnerText.hashKey, objInnerText.httpReturnCode);
}
...
var wasCrossDomainHTTPRequestDataReturned = function (response, hashKey, httpReturnCode) {
if (_BCAPIHelper && _BCAPIHelper.callbacksHash[hashKey]) {
...
var tmpElement = document.createElement(tmpElementName);
tmpElement.innerHTML = str;
var retStr = tmpElement.textContent || tmpElement.innerText;
tmpElement = null;
return retStr;
}
_BCAPIHelper.callbacksHash[hashKey](decodeEntities(response), httpReturnCode);
}
};
...
this.crossDomainHttpRequest = function (hashKey, method, url, postParams, userName, password, strJsonHeaders, window) {
...
return communicate.registerForMessaging(key);
else
return UNSUPPORTED;
};
...
return communicate.injectScript(strScript, iTabId, bInjectToIframes);
else
return UNSUPPORTED;
};
...
this.CrossDomainHttpRequest = function (callback, method, url, postParams, userName, password, headersArr) {
var strJsonHeaders = "",
arrOfHeaders = [];
method = method || "GET";
if (!url || !callback)
...
apiPermissions: conduit.currentApp.apiPermissions
});
}
...
return { result: '', status: 9999, description: SendRequestError };
}
},
onSysReq: {
...
if (response.data.data && response.data.topicName === strTopicName) {
callback.apply(this, [response.data.data, response.data.senderName || null, callback]);
}
}
...
var finalTopicName = targetMethod === "advanced.messaging.postTopicMessage"
? "adv:" + topicName
: ":" + topicName;
...
conduit.storage.app.keys.get = function (keyName, callback) {
try {
var data = JSON.parse(npapiPlugin.getKey(conduit.currentApp.info.toolbar.cID, getStorageKey("app", encodeURI(keyName)))).result;
if (callback) callback(data);
return data;
}
catch (generalException) {
...
conduit.storage.app.items.get = function (keyName, callback) {
try {
var data = JSON.parse(npapiPlugin.getData(conduit.currentApp.info.toolbar.cID, getStorageKey("app", encodeURI(keyName)))).result;
if (callback) callback(data);
return data;
}
catch (generalException) {
...
conduit.storage.global.keys.get = function (keyName, callback) {
try {
var data = JSON.parse(npapiPlugin.getGlobalKey(conduit.currentApp.info.toolbar.cID, getStorageKey("global", encodeURI(keyName)))).result;
if (callback) callback(data);
}
catch (generalException) {
...
try {
var ret = npapiPlugin.setKey(conduit.currentApp.info.toolbar.cID, getStorageKey("app", encodeURI(keyName)), data);
...
try {
var ret = npapiPlugin.setGlobalKey(conduit.currentApp.info.toolbar.cID, getStorageKey("global", encodeURI(keyName)), data);
if (ret && !ret.status && window.EBGlobalKeyChanged) {
EBGlobalKeyChanged(keyName, data);
}
return ret;
}
catch (generalException) {
...
try {
return npapiPlugin.setData(conduit.currentApp.info.toolbar.cID, getStorageKey("app", encodeURI(keyName)), data);
}
catch (generalException) {
if (window.console)
console.error("General Exception: " + generalException + " " + (generalException.stack ? generalException.stack.toString() : "") + document.location);
...
setTimeout(function () {
// Enable EBKeyChanged:
conduit.storage.app.keys.onChange.addListener(function (key, value) {
try {
...
conduit.storage.global.keys.onChange.addListener(function (key, value) {
try {
...
this.changeHeight = function (newHeight) {
var featuresObj = this._getFeaturesObj(RetrieveKey("APP_WIN_FEATURES"));
...
password: password,
headers: headersArr
}, _BCAPIHelper.callbacksHash[callbackHashKey]);
};
...
: document.documentElement ? document.documentElement.offsetWidth : undefined;
};
this.getHeight = function () {
//var featuresObj = this._getFeaturesObj(RetrieveKey("APP_WIN_FEATURES"));
// roee ovadia 12.12.11 check if embeded app return null object;
var featuresObj = conduit.currentApp.context === "embedded" ? {} : this._getFeaturesObj(RetrieveKey("APP_WIN_FEATURES"));
var isTitleEnabled = featuresObj.titlebar;
var isResizeEnabled = featuresObj.resizable;
var height;
if (navigator.userAgent.indexOf("MSIE") != -1) {
...
this.getMainFrameTitle = function () {
return data.currentTab && data.currentTab.title;
};
this.getMainFrameUrl = function () { return data.currentTab && data.currentTab.url; };
...
this.openGadget3 = function (url, width, height, features) {
var featuresObj = this._getFeaturesObj(features);
...
this._getFeaturesObj = function (features) {
if (!features) {
return {};
}
...
this.sendMessage = function (key, data) { conduit.messaging.postTopicMessage(key, data); };
