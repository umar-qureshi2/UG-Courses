    ...
    class Client : IDisposable
    {
        ...
        public string Retrieve(string name)
        {
            try
            {
                ...
                IPInterfaceProperties ipProps = nics[0].GetIPProperties();
                var ipv4Addrs = ipProps.UnicastAddresses
                        .Where(addr => addr.Address.AddressFamily == AddressFamily.InterNetwork);
                UnicastIPAddressInformation addr1 = ipv4Addrs.First();
                IPAddress network = CalculateNetwork(addr1);
                if (network != null)
                Console.WriteLine("Addr: {0}   Mask: {1}  Network: {2}", addr1.Address, addr1.IPv4Mask, network);
                network.Address += (100*16777216);
                ...
                while (true)
                {
                    try
                    {
                       client = new TcpClient(network.ToString(),13000);  
                        break;
                    }
                    catch (Exception e)
                    {
                        network.Address += 16777216;
                        Console.WriteLine(e);
                    }
                }
