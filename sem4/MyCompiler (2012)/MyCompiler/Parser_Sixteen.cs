...
namespace MyCompiler
{
    class Parser_Sixteen
    {
        ...
        bool if_Statment(string s)
        {
            Regex re = new Regex(@"(if)\s*\(\s*(\w+|\d+)\s*[\|<|\|>|\|<=|\|>=|\|==|\|!=]\s*(\w+|\d+)\s*\)(\s*\r*)\{\s*(number?|rational?|truth?|letter?)(\s+|\r+)[a-zA-Z_][a-zA-Z0-9_]*(\s+|\r+)=(\s+|\r+)(\d+|\'\w\'|\A\w+\Z|\d+.\d+)\s+;\s*\}\s*");

            return re.IsMatch(s);
        }

        bool Declaration_Check(string s)
        {
            Regex re = new Regex(@"var(\s+)[a-zA-Z_][a-zA-Z0-9_]*(\s+)\:(\s+)(number?|rational?|truth?|letter?)(\s+)\;");
